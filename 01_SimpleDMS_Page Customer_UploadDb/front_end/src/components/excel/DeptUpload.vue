...
<script>
import DeptExcelDataService from "../../services/DeptExcelDataService";
export default {
    data(){
        return{
    ...
    methods: {
        // 조회 함수
        retrieveExcel() {
        DeptExcelDataService.getFiles()
            // 성공하면 .then() 결과가 전송됨
            .then((response) => {
            this.dept = response.data; // 스프링부트에서 전송한 데이터
            // 디버깅 콘솔에 정보 출력
        ...
        upload(){
            DeptExcelDataService.upload(this.currentImage)
            .then((response) => {
        ...
        // 어려움 !!
        download(){
            DeptExcelDataService.download() // backend download 엑셀파일 요청
            // backend 성공하면 .then() 결과가 전송됨 (response.data)
            .then((response) => {
                console.log(response);
                // 첨부파일(엑셀, pdf, etc) 다운로드 기능 구현
                // 이미지 다운로드 : <img src="이미지URL">, 또는 <a href="이미지URL">이미지 URL</a>
                // 첨부파일 다운로드 : <a href="엑셀파일URL">...URL</a>
                // js 에서 강제로 a 태그를 생성하고 a 태그 강제 클릭 시킴 a 태그 삭제
